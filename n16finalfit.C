{
  ...
  // Or not.
  // Require that the muon be reconstructed to stop near the acrylic,
  // then assume it actually stopped in the acrylic, find the point
  // where it stopped if that is true, and find the distance from there
  // to the decay, then require that distance be small. Ok, then?
  //" && distfromacrylic(mx, my, mz) < 200";
  const char * const acrcut = "1"; /*pow(dx - nearestax(mx, my, mz), 2) +"
                              "pow(dy - nearestay(mx, my, mz), 2) +"
  ...
  const char * xscut = string(Form("%s && dt > 4e3 && dt < 30e3 && %s && %s && %s",
...
*/
 ...
 // What is the be11 efficiency?
  ...
  TF1 * ee = new TF1("ee",
               Form("%f*[0]/7.13*log(2)*exp(-x*log(2)/[1]) + " // n16
...
/*  TF1 gaus("gaus", "gaus(0)", 0, 20);
  ...
  const string escut = Form("%s && %s && %s && %s && dt > 4e3 && dt < 30e3",
  ...
  const string b12scut = Form("%s && %s && %s && %s && dt > 4e3 && dt < 30e3",
  ...
  gMinuit->Command("mncont 1 6 100");

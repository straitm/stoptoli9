...
/*  c->Divide(2, 1);
  ...
  const char * const cute = "e > 5.0 && e < 9.0";
...
//  c->cd(1)->cd(1);
  ...
  const double c15eff = 1
    * 0.981 // subsequent muons
    * 0.977 // previous muons
    * 0.565 // delta r for 200mm
    * 0.9709 // 100s from end of run
    * 0.449 // energy, estimated from scaled n16 MC
    * 0.986 // from ttlastvalid cut, very naive
    * 0.96 // from ttlastmuon cut, vary naive
    * (1-0.00504 * 20.20/178.3) // from b12like cut, using li-9 as a guide
  ...
  printf("%sN-16 eff: %f; C-15 eff: %f%s\n", RED, n16eff, c15eff, CLR);

  TF1 * ee = new TF1("ee",
               Form("%f*(%f*[0]/7.13*log(2)*exp(-x*log(2)/7.13) + " // n16
               "[1]*exp(-x*log(2)/0.0202) + " // b12
               "[2]*exp(-x*log(2)/0.8399) + " // li8
               "[3] +" // accidentals
               "%f*[4]/2.449*log(2)*exp(-x*log(2)/2.449))", // c15
               hfit->GetBinWidth(1), n16eff, c15eff), 0, 100);

  ee->SetParameters(0.1, 7.13, 3000, 2, 0.1, 1);
  ee->FixParameter(0, 0);
  ee->FixParameter(4, 0);
  if(fit) printf("fitting...\n"), hfit->Fit("ee", "l");  // fit with no n16 or c15

  double likenon16 = 0; if(fit) likenon16 = gMinuit->fAmin;
  ee->ReleaseParameter(0);
  ee->ReleaseParameter(4);
  ee->SetParLimits(0, 0, 2000);
  ee->SetParLimits(4, 0, 2000);
  if(fit) printf("fitting...\n"), hfit->Fit("ee", "l"); // fit with n16 and c15

  double likewn16 = 0; if(fit) likewn16 = gMinuit->fAmin;

  if(likenon16 > likewn16)
    printf("%sSignificance for n16 and/or c15 rather than neither %f%s\n", RED,
           sqrt(2)*sqrt(likenon16-likewn16), CLR);

  ee->FixParameter(0, 0);
  if(fit) printf("fitting...\n"), hfit->Fit("ee", "le");  // fit with not n16 and c15
  ...
  ee->ReleaseParameter(0);
  ee->FixParameter(4, 0);
  if(fit) printf("fitting...\n"), hfit->Fit("ee", "le");  // fit with n16 and no c15
  ...
  TF1* c15= new TF1("c15",Form("%f*(%f*[0]/2.449*log(2)*exp(-x*log(2)/2.449) + [1])",hfit->GetBinWidth(1), c15eff),0,100);
  ...
  TF1 * parts[5] = { b12, li8, n16, acc, c15 };

  n16->SetParameter(0, eedisp->GetParameter(0));
  n16->SetParError(0, eedisp->GetParError(0));
  n16->SetParameter(1, eedisp->GetParameter(3));
  n16->SetParError(1, eedisp->GetParError(3));

  b12->SetParameter(0, eedisp->GetParameter(1));
  b12->SetParError(0, eedisp->GetParError(1));
  b12->SetParameter(1, eedisp->GetParameter(3));
  b12->SetParError(1, eedisp->GetParError(3));

  li8->SetParameter(0, eedisp->GetParameter(2));
  li8->SetParError(0, eedisp->GetParError(2));
  li8->SetParameter(1, eedisp->GetParameter(3));
  li8->SetParError(1, eedisp->GetParError(3));

  c15->SetParameter(0, eedisp->GetParameter(4));
  c15->SetParError(0, eedisp->GetParError(4));
  c15->SetParameter(1, eedisp->GetParameter(3));
  c15->SetParError(1, eedisp->GetParError(3));

  acc->SetParameter(0, eedisp->GetParameter(3));
  acc->SetParError(0, eedisp->GetParError(3));
  ...
  for(int i = 0; i < 5; i++){
    ...
    if(nearlimit[i]){
      printf("plot %d is near limit\n", i);
      parts[i]->SetParameter(0, c15norm*mult); // XXX parts[i]->GetParameter(0)+
  ...
  printf("%sN N-16 found assuming no C-15: %f +%f %f %s%s\n",
         RED, Nfound, Nerrup, Nerrlo, errtype, CLR);

  const double Ocaptures = (0.2 + 5.67 + 0.3 + 1.4) * 489.509;

...
const double neff = (neff_dr_800_gd*   b12gamma215targfraction
                 ...
                 // Assumed to be substantially better after FE upgrades
                   * 0.9467 /* XXX neff_dt_215MeV <-- 0.92 */;
...
const double eff_eor_100s  = (livetime_s - num_runs*100.)/livetime_s;
...
const double fq_per_mev = 1/9.25e-5;

const double b12ecutlow = 3;

const double distcut = 800;
...
void fcn(int & npar, double * gin, double & chi2, double *par, int flag)
{
  ...
  for(int i = 0; i < ehist->GetNbinsX(); i++){
 ...
 
    {
      ...
      const double theory = ggn->Eval(bincenter);
...
void near_b12gamma_finalfit(const int region = 1, const int whichcorr_ = 0, double targfrac = 0)
{
  if(region < 0){
  ...
  const double lowt   = region == 0? 1.024 :region == 1?  1.024: 1.024;
  const double hight = 5.024; // us
  const double highfq = 1000; // MeV
  ...
  const double b12eff = 1
    ...
    * (distcut == 400?wholedet_dist400eff:1 /* XXX */)
    * (exp(-b12lowt /b12life)
  ...
  TFile * f = new TFile("/cp/s4/strait/ndfido/foo.root", "read");
  ...
  char filename[100];
  strcpy(filename, "/tmp/b12gamma.XXXXXX");
  ...
  TF1 *bggg = new TF1("bggg",
    ...
    "[0]+gaus(1)+gaus(4)+[7]*(3*(x/53.6)^2-2*(x/53.6)^3)",0,10);
  bggg->SetNpx(400);
  ...
  corrbgfit->SetParLimits(0, 0, 100);
  ...
  for(int i = 3; i <= 13; i+=2){
    mn->Command(Form("set limits %d 0.1 100.", i+1));
    mn->Command(Form("set par %d 3", i+1));
  }
  for(int i = 31; i <= 36; i++){
    mn->Command(Form("set limits %d 0.1 100.", i+1));
    mn->Command(Form("set par %d 1", i+1));
  }
  ...
  ehist->GetYaxis()->SetRangeUser(0.0001, 150);
